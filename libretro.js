var BrowserFS=BrowserFS;var afs;var initializationCount=0;function cleanupStorage()
...
{if(e)
{afs=new BrowserFS.FileSystem.InMemory();console.log("WEBPLAYER: error: "+e+" falling back to in-memory filesystem");setupFileSystem("browser");appInitialized();}
else
...
{$('#icnLocal').removeClass('fa-spinner').removeClass('fa-spin');$('#icnLocal').addClass('fa-check');console.log("WEBPLAYER: idbfs setup successful");setupFileSystem("browser");appInitialized();}
function appInitialized()
{initializationCount++;if(initializationCount==2)
{preLoadingComplete();}}
...
{$('#btnAdd').addClass('disabled');$('#icnAdd').removeClass('fa-plus');$('#icnAdd').addClass('fa-spinner spinning');var count=files.length;for(var i=0;i<count;i++)
...
var Module={noInitialRun:true,arguments:["-v","--menu"],preRun:[],postRun:[],onRuntimeInitialized:function()
{appInitialized();},print:function(text)
